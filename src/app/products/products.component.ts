...
export class ProductsComponent implements OnInit {
  ...
  totalProductInStock :number = this.products.length - this.products.filter(p => p.available === 'Available').length;
  totalProductOutStock :number = this.products.length - this.products.filter(p => p.available === 'Not Available').length;
